    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-background via-primary/5 to-background">
        <Card className="p-8 text-center">
          ...
          <Link href="/login">
            <Button className="bg-primary">Zum Login</Button>
          </Link>
        </Card>
      </div>
    );
  ...
  return (
    <main className="min-h-screen bg-gradient-to-br from-background via-primary/5 to-background py-12">
      <div className="container mx-auto px-4">
        ...
        {/* Info-Box */}
        <Card className="mb-8 border-2 border-primary/30 bg-primary/10">
          <CardContent className="p-6 flex items-start gap-4">
            <CheckCircle className="w-8 h-8 text-primary flex-shrink-0 mt-1" />
            <div>
              <h3 className="font-semibold text-primary mb-1">Lernpfad</h3>
          ...
          {loading ? (
            ...
            Object.entries(lernzieleNachSemester)
              ...
              .map(([semester, lernziele]: [string, Lernziel[]]) => (
                <Card key={semester} className="border-2 border-primary/30 shadow-lg">
                    ...
                    <div className="grid md:grid-cols-2 gap-4">
                      {lernziele.map(lz => (
                        <Link key={lz.id} href={`/lernziel/${lz.slug || lz.id}`}>
                          <Card className="h-full hover:shadow-lg transition-all border-2 border-gray-200 hover:border-primary">
        ...
        <Card className="mt-8 border-2 border-primary/30 bg-gradient-to-r from-primary/10 to-primary/5">
